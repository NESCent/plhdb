...
public class LoginController implements Controller {
    ...
    public ModelAndView handleRequest(HttpServletRequest request,
		...
		try{
			...
			UserAccount ua = (UserAccount) session
				.createCriteria(UserAccount.class).add(
			    ...
			    log()
				    .error(
					    "failed to find the roles for the logged in user: "
						    + email);
			    throw new IllegalArgumentException(
				    "failed to find the roles for the logged in user: " + email);
			} else {
			...
			// prepare commonly used data in the editing interface, such as cvterms,
			...
			result = q.list();
			for (int i = 0; i < result.size(); i++) {
			    stops.add((Cvterm) ((Object[]) result.get(i))[0]);
			}
			...
			result = q.list();
			for (int i = 0; i < result.size(); i++) {
				birthdayDists.add((Cvterm) ((Object[]) result.get(i))[0]);
			}
